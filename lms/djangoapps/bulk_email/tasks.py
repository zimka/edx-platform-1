...
def _add_unsubscribe_email_text(html_msg, plaintext_msg, email, course_email):
    unsub_headers = {}
    ...
    _html_msg = u'{0}Для отписки от рассылки курса перейдите <a href="{1}">по ссылке.</a>'.format(
        html_msg, unsub_url)
    _plaintext_msg = u'{0}Для отписки от рассылки курса перейдите по ссылке {1}.'.format(
        plaintext_msg, unsub_url)
    return _html_msg, _plaintext_msg, unsub_headers
...
def _send_course_email(entry_id, email_id, to_list, global_email_context, subtask_status):
    ...
    try:
        ...
        while to_list:
            ...
            to_course_html_msg = u'<br/><p>Вы получили это письмо, потому что записаны на курс "{0}" на ' \
                                 u'платформе "Открытое образование". Чтобы продолжить обучение, перейдите ' \
                                 u'<a href="{1}courseware">по ссылке.</a></p>'.format(
                course_title, 
                global_email_context['course_url']
            )
            to_course_plaintext_msg = u'Вы получили это письмо, потому что записаны на курс "{0}" на платформе ' \
                                      u'"Открытое образование". Чтобы продолжить обучение, ' \
                                      u'перейдите по по ссылке {1}courseware.'.format(
                course_title, 
                global_email_context['course_url']
            )
            ...
            html_msg, plaintext_msg, unsub_headers = _add_unsubscribe_email_text(html_msg, plaintext_msg, email, course_email)
            # Create email:
